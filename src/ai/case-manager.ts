...
import { StoryManager, type PairAction, type SpeakerCandidate, type SpeechDraft } from "./story-manager";
...
export class CaseManager {
  ...
  /** Map of character name → WebSocket user ID for pairing */
  private characterUserIdMap = new Map<string, string>();
  ...
  /**
   * Register the WebSocket user ID for a character (used for pairing).
   */
  setCharacterUserId(characterName: string, userId: string): void {
    this.characterUserIdMap.set(characterName, userId);
  }
  ...
  async nextBeat(options: NextBeatOptions): Promise<NextBeatResult> {
    ...
    // Handle autonomous pairing request from AI
    if (refined.pairAction) {
      this.handlePairAction(refined.pairAction, character);
    }
  ...
  /**
   * Handle an autonomous pairing request generated by the AI during speech.
   */
  private handlePairAction(pairAction: PairAction, speaker: CharacterManager): void {
    if (!this.masterSocket) {
      console.warn("[pair] No master socket available to perform pair action");
      return;
    }

    if (pairAction.action === "unpair") {
      console.log(`[pair] AI requested unpair for ${speaker.name}`);
      this.masterSocket.leavePair();
      this.masterSocket.sendPlainMessage({
        text: `[System] ${speaker.name} steps away.`
      });
      return;
    }

    if (pairAction.action === "pair" && pairAction.targetCharacterName) {
      const targetName = pairAction.targetCharacterName;
      const targetProfile = Array.from(this.characters.values()).find(
        (c) => c.name.toLowerCase() === targetName.toLowerCase(),
      );

      if (!targetProfile) {
        console.warn(`[pair] AI requested pair with "${targetName}" but character not found`);
        return;
      }

      // Resolve WebSocket user IDs for both characters
      const speakerUserId = this.characterUserIdMap.get(speaker.name);
      const targetUserId = this.characterUserIdMap.get(targetProfile.name);

      if (!speakerUserId || !targetUserId) {
        console.warn(`[pair] Could not resolve user IDs for ${speaker.name} or ${targetProfile.name}`);
        return;
      }

      console.log(`[pair] AI requested pair: ${speaker.name} -> ${targetProfile.name}`);
      this.masterSocket.createPair({ invitedUserIds: [speakerUserId, targetUserId] });
      this.masterSocket.sendPlainMessage({
        text: `[System] ${speaker.name} moves to stand with ${targetProfile.name}.`
      });
    }
  }
